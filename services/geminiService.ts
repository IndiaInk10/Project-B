...
export const generateBuildIdeas = async (inventory: InventoryItem[]): Promise<BuildIdea[]> => {
  ...
  try {
    const inventoryList = inventory.map(item => `- ${item.quantity}x ${item.part.name}`).join('\n');
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        systemInstruction: "You are a strict LEGO logic engine. You never over-promise. You prioritize builds that use 100% existing inventory over complex builds that require missing parts."
      }
    });
...
export const generateLDrawScript = async (ideaTitle: string, inventory: InventoryItem[]): Promise<string> => {
  ...
  try {
    ...
    const prompt = `
      ...
      STRICT CONSTRAINTS:
      1. Use ONLY the following available parts if possible: ${partsList}.
      2. If you absolutely need a connector to make the physics work, use a standard brick (3001) or plate (3020), but keep it minimal.
      3. Arrange the parts to look like: ${ideaTitle}.
      4. Output ONLY raw LDraw text.
